...
struct INodeContainer
{
	...
	virtual void ForEachChild(const ForEachFunctorType& f) const = 0;
	virtual NodePtr FindChild(const std::string& name) const = 0;
	virtual NodePtr FindChildIf(const FindIfFunctorType& f) const = 0;
